...
sequence_data = individuals %>% 
  filter(condition == "Lo Dep (B1) Lo Sat (B2)" & trial %in% 3:(3+n_seq_trials) |
         condition == "Hi Dep (B1) Lo Sat (B2)" & trial %in% 44:(44+n_seq_trials) |
...
jpeg(here("Plots", "Figure 3 - Person-Level Sequential Demand Curves.jpeg"),
  ...
  sequential_demand %>% 
    ...
    mutate(transaction_image = case_when(transaction == "Successful" ~
    ...
    ggplot(aes(x = log_price, color = condition_lab)) +
      # GEOMS
      geom_image(aes(x = log10(.045), y = 12.5, image = transaction_image), size = .2,
                 inherit.aes = F) +
...
## Color Ind Trials
```{r, eval = T}
jpeg(here("Plots", "Figure 4 (Color Ind Trials) - Person-Level Purchase Probabilities.jpeg"),
  ...
  individuals %>% 
    ...
    ggplot(aes(x = trial, y = prob.buy, color = condition_lab)) +
      ...
      geom_point(aes(shape = transaction, size = transaction),
                 fill = "white", color = "black", alpha = .5) +
      geom_line(data = make_condition_label(filter(sequence_data, new_trial < n_seq_trials),
                                            stacked = F)) +
      geom_point(data = make_condition_label(filter(sequence_data, new_trial < n_seq_trials),
                                             stacked = F),
...
## Black Ind Trials
```{r, eval = T}
jpeg(here("Plots", "Figure 4 (Black Ind Trials) - Person-Level Purchase Probabilities.jpeg"),
       width = 7, height = 13, units = "cm", res = 300)
  individuals %>% 
    make_condition_label(stacked = F) %>%
    ggplot(aes(x = trial, y = prob.buy, color = condition_lab)) +
      ...
      geom_line(data = make_condition_label(filter(sequence_data, new_trial < n_seq_trials),
                                            stacked = F), color = "black", alpha = .5) +
      geom_point(data = make_condition_label(filter(sequence_data, new_trial < n_seq_trials),
                                             stacked = F),
                 aes(shape = transaction, size = transaction), fill = "white", color = "black") +
      # scales
      scale_shape_manual(values = c(21, 16)) +
      scale_size_manual(values = c(1, 1.25)) +
      scale_x_continuous(limits = c(0,200),
...
jpeg(here("Plots", "Figure 5 (No Aggregating) - Population-Level Purchase Probabilities.jpeg"),
  ...
  population %>% 
    make_condition_label(stacked = F) %>%
    ggplot(aes(x = trial, y = prob.buy, 
               color = condition_lab, fill = condition_lab, group = simulation)) +
      # geoms
      geom_line(alpha = .25, linewidth = .1) +
...
## Grp Faded
```{r, eval = T}
jpeg(here("Plots", "Figure 5 (Grp Faded) - Population-Level Purchase Probabilities.jpeg"),
       width = 7, height = 13, units = "cm", res = 300)
  population %>% 
